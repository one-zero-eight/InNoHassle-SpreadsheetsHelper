...
namespace CommonLinter {
  ...
  lintCommon = Profiler.wrap(lintCommon);

  export let fixSpaces = () => {
    return _fixSpaces(Current.getTargetRange(), Current.getTargetValues());
  };

  fixSpaces = Profiler.wrap(fixSpaces);

  let _lintCyrillicSymbols = (
    value: string,
  ...
  let _lintSpacesNearBrackets = (
    ...
    if (value.match(noSpaceBeforeBracketPattern)) {
      warnings.push({
        content:
          "Space before bracket not found in cell " + cell.getA1Notation(),
        range: cell.getA1Notation(),
      });
    }
  ...
  let _fixSpaces = (
    ...
    for (let row = 0; row < values.length; row++) {
      for (let column = 0; column < values[row].length; column++) {
        ...
        const cell = range.getCell(row + 1, column + 1);
        if (
          value.match(leadingSpacePattern) ||
